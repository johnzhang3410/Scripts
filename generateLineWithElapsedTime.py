...
try:
    ...
    df = pd.read_csv(
        ...
        names=['Elapsed Time (minutes)', 'Total Memory (GB)'],
...
df_clean['Elapsed Time (minutes)'] = pd.to_numeric(df_clean['Elapsed Time (minutes)'], errors='coerce')
df_clean['Total Memory (GB)'] = pd.to_numeric(df_clean['Total Memory (GB)'], errors='coerce')

# Drop any rows that couldn't be converted to numeric values (only dropping rows where 'Elapsed Time (minutes)' is NaN)
df_clean = df_clean.dropna(subset=['Elapsed Time (minutes)'])

# Convert Time to Seconds and Add a New Column to df_clean
time_in_seconds = []
for minute, group in df_clean.groupby('Elapsed Time (minutes)'):
...
df_clean['Elapsed Time (seconds)'] = time_in_seconds
...
plt.title('16 core planner build RAM distribution')
...
max_time_in_seconds = df_clean['Elapsed Time (seconds)'].max()
